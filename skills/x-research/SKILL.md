...
user-invocable: true
metadata: {"openclaw":{"emoji":"🛰️","always":true,"os":["win32","linux","darwin"]}}
---
...
Prereqs:
- Install **Bun** so `bun` is on PATH: https://bun.sh
- Set `X_BEARER_TOKEN` (read-only token recommended)

Token sources (checked in this order):
1. `X_BEARER_TOKEN` env var
2. `X_RESEARCH_ENV_FILE` (path to an env file)
3. `~/.config/env/global.env` (Windows: `%USERPROFILE%\\.config\\env\\global.env`) containing `X_BEARER_TOKEN=...`

All commands run from this skill directory (`{baseDir}`):
...
- `--save` — save results to OpenClaw workspace `drafts/` (fallback: `~/clawd/drafts/`)
...
On heartbeat, can run `watchlist check` to see if key accounts posted anything notable. Flag to Jordan only if genuinely interesting/actionable — don't report routine tweets.
