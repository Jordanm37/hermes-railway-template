 ...
 *   --save                     Save results to OpenClaw workspace drafts/ (or ~/clawd/drafts/)
...
function getOpenclawWorkspace(): string | null {
  const envWs = process.env.OPENCLAW_WORKSPACE || process.env.CLAW_WORKSPACE;
  if (envWs) return envWs;

  const home = getHomeDir();
  if (!home) return null;

  const ws = join(home, ".openclaw", "workspace");
...
function getDraftsDir(): string {
  const ws = getOpenclawWorkspace();
  if (ws) return join(ws, "drafts");

  const home = getHomeDir();
  if (home) return join(home, "clawd", "drafts");

  // fallback: drafts under the skill directory
  return join(SKILL_DIR, "drafts");
}

const DRAFTS_DIR = getDraftsDir();
...
Search options:
  ...
  --no-replies               Exclude replies
  --save                     Save to OpenClaw workspace drafts/ (or ~/clawd/drafts/)
