...
function readTokenFromEnvFile(envPath: string): string | null {
  try {
...
function getToken(): string {
  ...
  // Explicit override
  if (process.env.X_RESEARCH_ENV_FILE)
    candidates.push(process.env.X_RESEARCH_ENV_FILE);

  // Default global.env
  const home = getHomeDir();
  if (home) candidates.push(join(home, ".config", "env", "global.env"));

  for (const p of candidates) {
    const token = readTokenFromEnvFile(p);
    if (token) return token;
  }

  throw new Error(
    "X_BEARER_TOKEN not found. Set X_BEARER_TOKEN env var (recommended) or create a global.env file at ~/.config/env/global.env (Windows: %USERPROFILE%\\.config\\env\\global.env)."
  );
}
